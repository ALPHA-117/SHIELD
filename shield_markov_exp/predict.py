...
from .weather import fetch_openmeteo_rain
...
def predict_flood(
    ...
    if future_rain_list is not None:
        ...
        # Fetch real forecast from Open-Meteo (Tier 1: Days 1-7)
        api_days = min(7, future_days)
        try:
            from datetime import timedelta as _td
            forecast_start = (last_date + _td(days=1)).date() \
                if hasattr(last_date, "date") else last_date + _td(days=1)
            _prog(f"Fetching Open-Meteo forecast for {api_days} days (lat={lat:.3f}, lon={lon:.3f})…", 32)
            _resolved_rain_list = fetch_openmeteo_rain(lat, lon, forecast_start, days=api_days)
            _prog(f"✅ Open-Meteo API (Tier 1): {api_days} days fetched, "
                  f"total={sum(_resolved_rain_list):.1f} mm.", 34)
            if future_days > 7:
                _prog(f"⚠️ Beyond day 7 (Days 8-{future_days}): Falling back to SeasonalRainfallModel (Tier 2).", 35)
        except Exception as _api_err:
            log.warning("Open-Meteo fetch failed (%s) — falling back to SeasonalRainfallModel.",
                        _api_err)
            _resolved_rain_list = None   # will use rain_model below
    ...
    for day in range(1, future_days + 1):
        ...
        predictions.append((future_date, final_prob, risk_label, colour, next_rain))
...
def predictions_to_dataframe(predictions: List[PredictionRow]) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [(d.date(), round(p, 4), l, round(r, 4)) for d, p, l, c, r in predictions],
        columns=["date", "flood_probability", "risk_level", "predicted_rainfall_mm"],
    )
