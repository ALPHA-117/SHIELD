...
def create_features(df: pd.DataFrame, temp_c: float = 25.0) -> pd.DataFrame:
    ...
    # ── is_forecast: initialise for purely historical DataFrames ─────────────
    # If is_forecast is already present (e.g. rolling buffer from predict.py),
    # keep existing values so future rows tagged 1 are not zeroed out.
    # For fresh historical CSVs that have no such column, default to 0 (historical).
    ...
    # Use fill_value=0 only for columns other than is_forecast to preserve its values
