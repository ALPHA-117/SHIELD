...
def _augment_forecast_sequences(
    X_seqs: np.ndarray,
    y_seqs: np.ndarray,
    is_forecast_feat_idx: int,
    scaled_forecast_val: float,
    seq_len: int,
    rng: np.random.Generator,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Phase 2: Data-source-awareness augmentation.

    For each training sequence, generate one augmented copy where the
    last k steps (k drawn uniformly from [1, seq_len-1]) have the
    `is_forecast` feature set to the scaled value of 1.0.

    This teaches the LSTM that `is_forecast=1` rows carry uncertain
    rainfall inputs and the model should be less confident in forecast mode.
    Label y is preserved from the original historical ground truth.

    Parameters
    ----------
    X_seqs              : (n, seq_len, n_features) scaled sequence array
    y_seqs              : (n,) label array
    is_forecast_feat_idx: Column index of `is_forecast` in FEATURES
    scaled_forecast_val : Scaled representation of is_forecast=1.0
    ...
    for i in range(len(Xs_aug)):
        k = int(rng.integers(1, seq_len))   # [1, seq_len-1] steps to flip
        Xs_aug[i, -k:, is_forecast_feat_idx] = scaled_forecast_val
...
def train_pipeline(
    ...
    # Phase 2: expose is_forecast variance by fitting on a 50/50 mix of
    # is_forecast=0 (historical) and is_forecast=1 (synthetic forecast) rows.
    # Result: is_forecast mean=0.5, scale=0.5 → historical→-1.0, forecast→+1.0
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    combined_train = pd.concat(train_dfs)
    is_forecast_feat_idx = FEATURES.index("is_forecast")
    raw_train_vals = combined_train[FEATURES].values
    aug_scaler_vals = raw_train_vals.copy()
    aug_scaler_vals[:, is_forecast_feat_idx] = 1.0   # flipped to forecast
    scaler.fit(np.vstack([raw_train_vals, aug_scaler_vals]))
    # Compute the scaled representation of is_forecast=1.0 for sequence augmentation
    scaled_forecast_val = float(
        (1.0 - scaler.mean_[is_forecast_feat_idx]) / scaler.scale_[is_forecast_feat_idx]
    )
    log.info(
        "Phase 2 scaler: is_forecast mean=%.3f scale=%.3f → "
        "historical=%.2f, forecast=%.2f (in scaled space)",
        scaler.mean_[is_forecast_feat_idx], scaler.scale_[is_forecast_feat_idx],
        (0.0 - scaler.mean_[is_forecast_feat_idx]) / scaler.scale_[is_forecast_feat_idx],
        scaled_forecast_val,
    )
    ...
    # Phase 2: augment training sequences with forecast-mode examples (1:1 ratio)
    aug_rng = np.random.default_rng(RANDOM_SEED + 1)
    X_aug, y_aug = _augment_forecast_sequences(
        X_seq_tr, y_seq_tr,
        is_forecast_feat_idx=is_forecast_feat_idx,
        scaled_forecast_val=scaled_forecast_val,
        seq_len=SEQ_LENGTH,
        rng=aug_rng,
    )
    X_seq_tr_combined = np.vstack([X_seq_tr, X_aug])
    y_seq_tr_combined = np.concatenate([y_seq_tr, y_aug])
    log.info(
        "Phase 2 augmentation: %d original + %d forecast-mode seqs = %d total training seqs.",
        len(X_seq_tr), len(X_aug), len(X_seq_tr_combined),
    )

    # For XGBoost pass, we need the "all" sequences (no augmentation — uses last-row features)
    ...
    if len(X_seq_tr_combined) == 0:
        raise ValueError(
            f"Training set too small for sequence length {SEQ_LENGTH}. "
            "Need at least training_rows > SEQ_LENGTH."
        )

    # ── 7. Train LSTM (with Phase 2 forecast-mode augmentation) ───────────────
    _prog("Training LSTM (with forecast-mode augmentation)…", 42)
    ...
    lstm_model.fit(
        X_seq_tr_combined, y_seq_tr_combined,
